...
/**
 * iss_chain - test if current char in buffer is a chain delimeter
 ...
 */
int iss_chain(info_t *info, char *buf, size_t *p)
{
        size_t j = *p;
        ...
        for (i = 0; info->argv[i]; i++)
        {
                ...
                if (!str_comp(info->argv[i], "$?"))
                {
                        change_string(&(info->argv[i]),
                                string_dup(convert_no(info->status, 10, 0)));
                        continue;
                }
                if (!str_comp(info->argv[i], "$$"))
                {
                        change_string(&(info->argv[i]),
                                string_dup(convert_no(getpid(), 10, 0)));
                        continue;
                }
                ...
                {
                        change_string(&(info->argv[i]),
                                string_dup(_str_character(node->str, '=') + 1));
                        continue;
                }
                change_string(&info->argv[i], string_dup(""));
